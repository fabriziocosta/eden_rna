...
def rnashapes_wrapper(sequence, shape_type=None, energy_range=None,
                      max_num=None, rnashapes_version=None):
    """Fold sequence using RNAshapes."""
    # command line
    if rnashapes_version == 2:
        ...
        cmd = 'echo "%s" | RNAshapes --mode shapes \
        --shapeLevel %d --relativeDeviation %d' % (
            sequence, shape_type, energy_range)
        out = sp.check_output(cmd, shell=True)
        # parse output
        text = out.strip().split('\n')
        # remove additional fasta-like sequence id found in
        # RNAshapes version 3+
        text.pop(0)
        ...
        raise Exception("Error, parsing of RNAshapes version \
            level '{}' not implemented.".format(rnashapes_version))
...
def _string_to_networkx(header, sequence, **options):
    ...
    if split_components:
        for seq_struct, struct in zip(seq_struct_list, struct_list):
            ...
            energy_range=%s max_num=%s' % (shape_type,
                                           energy_range,
                                           max_num)
...
def fold(seqs, **options):
    ...
    return rnashapes_to_eden(seqs, **options)
